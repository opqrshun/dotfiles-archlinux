...
## Spec
- Title: dotfiles-shell を主ソースに、Layer1/Layer2責務分離した最小dotfilesを再構築する
- Status: Draft (Reset)

## Background
- `dotfiles-shell/` はシェル系設定を独立管理する意図で作られていた可能性が高い。
- `dotfiles-old/` は system/user 設定やアプリ固有設定が混在しており、最小化方針には過剰。
- 新構成では、シェル共通基盤を Layer2 に寄せつつ、Layer1 は OS/bootstrap のみに限定する。

## Goals
- `dotfiles-shell/` を中心に、再利用可能な shell/tmux 最小構成を定義する。
...
## Non-Goals
- `dotfiles-shell/` / `dotfiles-old/` の完全互換。
...
3. Layer2 は外部 `dotfiles-shell` リポジトリを実行する方式を採用し、管理責務を分離する。
...
- `dotfiles-shell/` には zsh + tmux + link スクリプトの核がある。
- `dotfiles-shell/install/setupShell.sh` は `yarn` 前提など依存が強い。
